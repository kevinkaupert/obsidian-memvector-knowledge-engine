...
- `term` provides human-readable natural language vocabulary (e.g. "implies", "follows from"). Mapping fine-grained conversational synonym phrases to canonical Cypher labels is preserved in `buildConversationalCategories` as a planned expansion (WIP tracked in Issue #43).
- `reversed` indicates when a natural phrase reverses graph direction (e.g. "follows from": A follows from B implies B -> A). The save pipeline (`resolveTerm.ts`) honors `reversed: true` during resolution; for active interactive editing, the Relation Builder UI provides an explicit "Richtung umkehren" (swap direction) button to reverse node topology visually before saving.

If the file doesn't exist yet, it's auto-created on first use of the Relation Builder with a bundled STEM preset (13 canonical labels across 37 terms - `relationVocabulary/defaultVocabulary.ts`) as a starting point. From then on the file is authoritative; the constant is never read again for that vault. For visual clarity, `drawEdges.ts` maps all 13 canonical relation labels to dedicated semantic colors (`CANONICAL_EDGE_COLORS`) across both ink and muted themes, while dynamically falling back to label string hashing for custom user-defined relation types.
