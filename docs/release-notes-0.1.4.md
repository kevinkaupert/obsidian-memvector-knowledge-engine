...
- **Transparent SQLite Persistence Feedback:** Persistence errors during manual or background vector recalculation are surfaced as `[ERROR]` notices, suppressing false `[OK]` success messages when vectors remain only in volatile WASM memory (#9, #48).
